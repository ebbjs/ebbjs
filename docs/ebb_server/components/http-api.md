...
| ------ | ------------------------ | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------- |
| `POST` | `/sync/handshake`        | Headers: auth token. Body: `{"cursors": {...}, "schema_version": N}` | `{"actor_id": "...", "groups": [{id, permissions, cursor_valid, reason, cursor}]}` | Authenticate, validate cursors, return Group list |
| `POST` | `/sync/actions`          | Body: MessagePack `{"actions": [...]}`                               | `{"rejected": [...]}` (200) or `{"error": "unauthorized"}` (401)                   | Write Actions. Blocks until durable.              |
| `GET`  | `/sync/groups/:group_id` | Query: `offset=<gsn>`                                                | JSON array of Actions + `Stream-Next-Offset` / `Stream-Up-To-Date` headers         | Paginated catch-up via `cf_group_actions` index   |
| `GET`  | `/sync/live`             | Query: `groups=A,B,C&cursor=500`                                     | SSE stream (`event: data`, `event: control`, `event: presence`)                    | Single SSE connection for live updates            |
...
See individual component docs for details on sync endpoint response shapes:

- **Handshake** — documented in [slice-4/handshake-update](../slices/slice-4/04-handshake-update.md)
- **Catch-up** — documented in [slice-4/catch-up-endpoint](../slices/slice-4/01-catch-up-endpoint.md)
- **Live SSE** — documented in [slice-4/live-sse-endpoint](../slices/slice-4/02-live-sse-endpoint.md)
- **Presence** — documented in [slice-4/presence-endpoint](../slices/slice-4/03-presence-endpoint.md)
...
| CatchUp            | Delegates to `CatchUp.catch_up_group/3` for catch-up reads | [slice-4/01-catch-up-endpoint](../slices/slice-4/01-catch-up-endpoint.md) |
...
See [slice-4/01-catch-up-endpoint](../slices/slice-4/01-catch-up-endpoint.md). The handler delegates to `CatchUp.catch_up_group/3`, which reads via the `cf_group_actions` pre-indexed column family.
