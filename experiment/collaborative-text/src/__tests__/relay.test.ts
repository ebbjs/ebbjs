 ...
 */
const makeInsertAction = (peerId: string, node: RunNode, hlc: Hlc): Action => {
...
describe("handleRemoteMessage — PRESENCE", () => {
  ...
  it("does not crash if updatePresence is not provided", () => {
    ...
    // Should not throw
    expect(() => handleRemoteMessage(message, peer.relayConfig)).not.toThrow();
  });
});
 ...
 */
const makePatchAction = (peerId: string, runId: string, appendText: string, hlc: Hlc): Action => {
  const id = `test_act_${testIdCounter++}`;
  return {
    id,
    actor_id: peerId,
