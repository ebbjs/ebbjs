 ...
 * `FieldValueFor<F>`.
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- phantom parameter for type narrowing at the call site
export interface EntityHandle<TFields extends Record<string, FieldMarker>> {
  readonly id: string;
...
async function namespaceCreate<TFields extends Record<string, FieldMarker>>(
  ...
  // The stub fetch didn't drive a corresponding inbound action, so
  // the materializer is empty. Return a placeholder Entity
  // carrying just the id and type so the call site doesn't
  // crash; the next `catchUp` / `readLocalEntity(id)` call will
  // hydrate the real material.
  return {
    id: entityId,
    type: entityName,
    data: { fields: {} },
 ...
 *
 * Implementation: each chain mutator delegates to the
 * underlying `query-builder.ts` chain (which is immutable: every
 * method returns a new builder carrying the new constraint),
 * then wraps the result so the terminals (`toArray` / `find`)
 * load candidates through the storage adapter.
 *
 * The wrapper holds the *plan* that the next chain call should
 * extend from. We capture that on entry and let the wrapper
 * pass it through to the underlying mutator. There is no
 * recursive bind — `chained.eq(...)` always resumes from the
 * plan captured at wrap-time, so a sequence like
 * `eq().orderBy().limit()` accumulates all three constraints.
 */
function namespaceFind<TFields extends Record<string, FieldMarker>>(
  ...
  // Hold the latest underlying plan in a closure-scoped variable.
  // The outer `let` is intentionally mutable; each `wrap(...)`
  // updates it for the next chained call.
