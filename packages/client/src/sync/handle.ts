 ...
 * ## Read consistency
 *
 * Own-field getters read from the client's sync snapshot. The
 * snapshot is populated lazily on the first read after a write
 * (`client.readLocalEntity(id)` materializes the entity in storage
 * and hydrates the snapshot). Users who need the post-update view
 * should call `client.readLocalEntity(id)` (or `catchUp`) first.
 * Future reactivity work (#161) will wire a true event-driven
 * snapshot.
 */
...
export function buildEntityHandle<TFields extends Record<string, FieldMarker>>(
  ...
  for (const rel of reverseRels) {
    Object.defineProperty(handle, rel.as, {
      ...
      enumerable: true,
      get() {
        return invokeReverse(client, entityName, entityId, rel.source.name, rel.as, rel.type);
      },
    });
  }
 ...
 *
 * Own fields are typed against `FieldValueFor<F>`; relationship
 * accessors are typed against the resolution shape
 * (`Promise<Entity | undefined>` for forward "one",
 * `QueryBuilder<TFields>` for forward "many" and for reverse).
 *
 * The `EntityHandle<TFields>` interface itself lives in
 * `namespace.ts` (it's the shape `EntityCollection<TFields>`'s
 * call signature returns). Re-exported above so the runtime
 * helper here can reference the canonical type.
 */

// Re-export `EntityHandle` from `namespace.ts` so callers that
// already import from `sync/handle` (e.g., the unit tests) don't
// need to chase the type across two files. The single source of
// truth lives in `namespace.ts` because that's where the
// `EntityCollection` interface — which references `EntityHandle`
// on the call-signature — is defined.
