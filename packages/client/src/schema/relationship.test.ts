...
describe("QueryBuilder (sync/relationship)", () => {
  ...
  it("eq filters by field equality", async () => {
    ...
    expect((out as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["1", "3"]);
  });
  ...
  it("orderBy sorts by field", async () => {
    ...
    expect((out as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["2", "1", "3"]);
  });
  ...
  it("limit caps the result count", async () => {
    ...
    expect((out as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["1", "2"]);
  });
  ...
  it("chains eq + orderBy + limit", async () => {
    ...
    expect((out as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["4", "3"]);
  });

  it("chains return new builders (no shared state)", async () => {
    const { buildQueryBuilder } = await import("../sync/query-builder");
    const rows: import("@ebbjs/core").Entity[] = [
      buildEntity("1", { y: false }),
      buildEntity("2", { y: true }),
    ];
    ...
    expect(((await a.find()) as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["2"]);
    expect(((await b.find()) as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual(["1"]);
    // And calling `.find()` on the base again returns everything.
    expect(((await base.find()) as import("@ebbjs/core").Entity[]).map((r) => r.id)).toEqual([
      "1",
      "2",
    ]);
  });
...
describe("relationship() handle traversal", () => {
  ...
  it("forward(id) returns a QueryBuilder for sourceCardinality:many", async () => {
    ...
    const qb = (await handle.forward("issue_1")) as {
      find(): Promise<readonly unknown[]>;
    };
    const out = (await qb.find()) as import("@ebbjs/core").Entity[];
