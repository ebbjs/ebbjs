 ...
 */
export type SyncMessage = { readonly type: "ACTION"; readonly action: Action } | PresenceMessage;
...
const applyUpdate = (update: Update, config: RelayConfig): void => {
  switch (update.method) {
    ...
    case "delete": {
      ...
      const deletePos = runOffsetToPosition(deleteDocState.index, update.subject_id, offset);
      ...
      if (deleteView && deletePos !== undefined) {
        applyRemoteDelete(deleteView, deletePos, count, config.idMapField, config.getDocState);
      }
      break;
    }
    ...
    case "patch": {
      ...
      const runStartPos = runOffsetToPosition(patchDocState.index, update.subject_id, 0);
 ...
 */
export const handleRemoteMessage = (message: SyncMessage, config: RelayConfig): void => {
  switch (message.type) {
    case "ACTION": {
