...
> **Note — Forward-looking API outline.** The server's soft-delete semantics (tombstones carrying `deleted_hlc` and `actor_id`) are **implemented** in `EntityStore` and `Writer`. The two-phase GC, configurable retention, and stale-cursor handling described below are **planned** — not implemented in either the server or the client.

The [Action](/docs/v1-target/data-model) log grows indefinitely without intervention. Ebb provides garbage collection (GC) to reclaim storage.

## Tombstones

When an Entity is deleted via a `DELETE` Update, Ebb doesn't remove it from storage. Instead, the Entity becomes a **tombstone**—a marker that the Entity was deleted. The tombstone retains the Entity's `id`, `type`, deletion timestamp ([HLC](/docs/v1-target/clock)), and the `actor_id` who deleted it. The `data` field (containing all [typed field](/docs/v1-target/data-model#typed-fields) values) is cleared.
...
2. **Conflict detection** — If a client edits an Entity offline while another client deletes it, the tombstone allows the first client to detect this [conflict](/docs/v1-target/conflicts) when they sync.
